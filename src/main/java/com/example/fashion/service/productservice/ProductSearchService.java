...
public class ProductSearchService {
    ...
    public List<ElasticSearchProduct> searchByKeyword(String keyword) throws IOException {
        Query matchFuzzy = MatchQuery.of(m -> m
        .field("name")
        .query(keyword)
        .fuzziness("2")   
)._toQuery();
        SearchResponse<ElasticSearchProduct> response = elasticsearchClient.search(s -> s
